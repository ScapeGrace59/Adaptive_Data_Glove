...
// Parameters for optimizing PI controller
...
// Timing for periodic optimization
unsigned long lastOptimizationTime = 0;
const unsigned long optimizationInterval = 500; // Interval for optimizing PI parameters (5 seconds)
...
void loop() {
  ...
  if (cal_gyro) {
    ...
    if (now_ms - last_ms >= print_ms) {
      ...
      Serial.print("0,0,0,0,0;"); // Placeholder values for other data
      ...
      Serial.print("0,0,0"); // Placeholder for other sensor data
      Serial.println("");

      // Periodically optimize PI parameters
      unsigned long currentTime = millis();
      if (currentTime - lastOptimizationTime >= optimizationInterval) {
        lastOptimizationTime = currentTime;
        optimizePIParameters(); // Perform optimization periodically
      }
    }
  }
}
...
// Function to compute the cost based on differences in yaw, pitch, and roll
float computeCost(float kp, float ki) {
  // Update the Mahony filter with the current PI parameters

  // Calculate the cost as the sum of the squared differences
  float cost = pow(yaw - lastYaw, 2) + pow(pitch - lastPitch, 2) + pow(roll - lastRoll, 2);

  // Store the current values as the last for the next iteration
  lastYaw = yaw;
  lastPitch = pitch;
  lastRoll = roll;
...
// Hooke and Jeeves Optimization algorithm to optimize Kp and Ki
void optimizePIParameters() {
  ...
  // Update the PI controller parameters
  Kp = bestKp;
  Ki = bestKi;
}